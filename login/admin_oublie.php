...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'send_token') {
        ...
        if ($check->rowCount() > 0) {
            ...
            $pdo->prepare("UPDATE admins SET reset_token = ? WHERE email = ?")->execute([$token, $email]);
            ...
            try {
                ...
                $mail->setFrom('no-reply@votre-mail', 'CSBJS Support');
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'reset_password') {
        ...
        if (!preg_match($regexPass, $new_pass)) {
            ...
            $stmt = $pdo->prepare("SELECT id FROM admins WHERE email = ? AND reset_token = ?");
            $stmt->execute([$email, $token_saisi]);

            if ($stmt->rowCount() > 0) {
                $hashed = password_hash($new_pass, PASSWORD_DEFAULT);
                $pdo->prepare("UPDATE admins SET mot_de_passe = ?, reset_token = NULL WHERE email = ?")->execute([$hashed, $email]);
                $msg = "<p style='color:green;'>Mot de passe modifié avec succès ! <a href='login_admin.php'>Se connecter</a></p>";
                $step = 3; 
            } else {
...
<body>
    <div class="login-box">
        ...
        <?= $msg ?>
        <br>
        <a class="back-link" href="login_admin.php">← Retour à la connexion</a>
    </div>
</body>
</html>